...
- (IBAction)okBTTap:(id)sender {
    if(self.customTipos.count <5 ){
        if ((self.textInput.text.length != 0) && !([self.textInput.text isEqual:@" "]) && (self.textInput.text.length < 20)){
            [GerenciadorBD criaType:self.textInput.text];
        }
        [self limpaERecria];
        
        [self.textInput setHidden:YES];
        [self.okBT setHidden:YES];
